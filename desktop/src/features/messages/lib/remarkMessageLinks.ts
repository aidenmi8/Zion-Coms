/**
 * Remark plugin that detects bare `zion://message?…` URLs in text nodes and
 * replaces each with a custom `message-link` HAST element. Legacy custom
 * scheme URLs remain accepted as input. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes only reach the `<a>` component override when the
 * user wrote an explicit labeled link.
 *
...
const MESSAGE_URL_PATTERN = /(?:zion|buzz):\/\/message\?[^\s<>"')\]]+/g;
