...
use super::{
    ...
    normalize_agent_args, parse_semver_tag, preset_catalog_entry, probe_codex_acp_major_version,
...
fn default_agent_command_resolves_bundled_zion_agent() {
    // The create-path default must be the bundled Zion agent, never the
    // bare `goose` that isn't on PATH on a stock Windows install.
    assert_eq!(default_agent_command(), "zion-agent");
    // And Zion Agent takes no `acp` arg — confirm no arg leakage from the default.
    assert_eq!(
