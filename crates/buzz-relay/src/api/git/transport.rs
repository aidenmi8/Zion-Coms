        ...
        RepoBinding::NotBound => {
            ...
            if repo_event.event.pubkey == *caller {
                ...
                return Err((
                    ...
                    format!(
                        "run: repos bind --id {repo_name} --channel <channel-uuid> — repository {repo_name:?} has no channel binding, so the relay cannot authorize access"
                    ),
                )
                    .into_response());
            }
...
mod sec005_read_gate_tests {
    ...
    async fn read_gate_gives_author_of_unbound_repo_remediation_body() {
        ...
        assert!(
            body.starts_with(&format!("run: repos bind --id {}", f.repo)),
