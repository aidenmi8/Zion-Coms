...
test.describe("community rail", () => {
  ...
  test("keyboard reorder: Space to pick up, ArrowUp to move, Space to drop updates stored order", async ({
    ...
    const movedAnnouncement = `Draggable item ${COMMUNITY_B.id} was moved over droppable area ${COMMUNITY_A.id}.`;
    await expect(
      page.locator('[role="status"]').filter({ hasText: movedAnnouncement }),
    ).toHaveText(movedAnnouncement);
