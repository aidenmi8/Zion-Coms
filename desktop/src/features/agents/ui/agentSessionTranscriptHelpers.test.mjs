...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // userText is empty and the title falls through to "Sion event".
  ...
  assert.equal(result.userTitle, "Sion event");
...
test("parsePromptText defaults the title to 'Sion event' when no kind is present", () => {
