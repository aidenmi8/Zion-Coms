...
use buzz_core::kind::{
    ...
    KIND_APPROVAL_DENY, KIND_APPROVAL_GRANT, KIND_APPROVAL_PASS, KIND_AUTH, KIND_BOOKMARK_LIST,
...
fn required_scope_for_kind(kind: u32, event: &Event) -> Result<Scope, &'static str> {
    match kind {
        ...
        KIND_APPROVAL_GRANT | KIND_APPROVAL_DENY | KIND_APPROVAL_PASS => Ok(Scope::MessagesWrite),
...
mod tests {
    ...
    #[test]
    fn approval_pass_requires_message_write_scope() {
        let event = make_dummy_event();
        assert_eq!(
            required_scope_for_kind(KIND_APPROVAL_PASS, &event).expect("pass scope"),
            Scope::MessagesWrite
        );
    }
