...
test("deleted public starter channels do not strand community onboarding", async ({
  ...
  await page.getByRole("button", { name: "Take me to Zion" }).click();
...
test("required Welcome creation failure keeps community onboarding open", async ({
  ...
  for (const [slug, name] of [
    ...
    const character = page.getByTestId(`starter-persona-${slug}`);
    await expect(character).toBeVisible();
    const presence = character.locator(
      '[data-brand-surface="starter-team-presence"]',
    );
    await expect(presence).toBeVisible();
    await expect(presence).toHaveAttribute("data-persona", name);
  ...
  const enterButton = page.getByRole("button", { name: "Take me to Zion" });
  await enterButton.click();

  await expect(page.getByTestId("community-onboarding-flow")).toBeVisible();
  await expect(page.getByText(`${welcomeError} Try again.`)).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Take me to Zion" }),
  ).toBeEnabled();
