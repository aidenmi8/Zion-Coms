...
const RELAY_MESH_RUNTIME_NO_TARGET: &str =
    "Sion shared compute requires a live serving member; start serving the selected model on a member, then try again";
...
fn mesh_readiness_failure_message(
    ...
    match failure {
        MeshReadinessFailure::CatalogNeverSynced => format!(
            "Sion shared compute connected to the serving member but could not sync \
        ...
        MeshReadinessFailure::RoutingNeverCompleted => format!(
            "Sion shared compute found \"{model_id}\" on a serving member but inference \
...
pub(crate) async fn ensure_client_node_for_model(
    ...
    let mut runtime = state.mesh_llm_runtime.lock().await;
    if runtime.is_some() {
        return Err("mesh node changed while starting Sion shared compute client".to_string());
    }
...
pub(crate) async fn ensure_relay_mesh_for_record(
    ...
    let target = match resolve_mesh_bootstrap_target(&state, &model_id).await {
        ...
        Ok(None) => {
            return Err(
                "Sion shared compute cannot start because no live member is serving this model. Start serving it on a member, then try again."
                    .to_string(),
            );
        }
        Err(error) => {
            return Err(format!(
                "could not refresh Sion shared compute serving members: {error}"
            ));
        }
    };
