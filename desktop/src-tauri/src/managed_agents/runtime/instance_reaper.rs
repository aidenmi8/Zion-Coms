...
/// Binary names for the Zion desktop/Tauri process. Legacy Buzz names remain
/// recognized so managed agents from older builds can still be reaped safely.
...
/// Check if a process name matches a known Zion or legacy Buzz desktop binary.
...
/// Check if a Zion desktop process is still alive for the given instance ID.
/// Scans all user-owned recognized desktop processes and checks
