...
grep -q '^APP_DISPLAY_NAME = Zion (Fix_Thing-2)$' "$ios" \
...
grep -Eq "^APP_DISPLAY_NAME = Zion \([A-Za-z0-9._-]+\)$" "$ios" \
...
grep -q 'resValue("string", "app_name", "Zion")' "$gradle" \
  && pass "Gradle default app_name stays Zion" \
