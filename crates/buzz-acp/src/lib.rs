...
mod agent_draft_prompt_tests {
    ...
    #[test]
    fn shared_base_prompt_uses_zion_for_product_facing_names() {
        let prompt = include_str!("base_prompt.md");
        assert!(prompt.starts_with("You are operating inside the Zion platform"));
        assert!(prompt.contains("## Zion CLI"));
        assert!(prompt.contains("call the service the **Zion relay**"));
        assert!(prompt.contains("The `buzz` executable"));
    }
