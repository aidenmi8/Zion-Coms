...
pub(crate) async fn rearm_relay_mesh_for_running_agents(app: &AppHandle) -> Result<(), String> {
    ...
    match recovery {
        ...
        MeshRuntimeRecovery::RestartRequired => {
            ...
            eprintln!(
                "buzz-mesh: supervised client startup lost its ingress before the SDK exposed a shutdown handle; restarting Zion"
            );
            app.request_restart();
            return Ok(());
        }
    ...
    for (record, mesh_model_id) in &mesh_records {
        ...
        .await
        {
            ...
            Err(error) => {
                let message = format!(
                    "{MESH_REARM_ERROR_SENTINEL}Zion shared compute offline — failed to re-arm local ingress for this agent: {error}"
                );
